...
- **User Profile Panel**: Dynamic sliding panel accessed from header icon with:
    - **Real-Time Shopping Statistics**: Displays total orders, total spent, items purchased, and average order value calculated from actual purchase data.
    - **Purchase History**: Shows last 10 completed orders with detailed item lists, quantities, prices, and order totals.
    - **User Preferences**: Toggleable settings for AI Recommendations and Budget Alerts with instant save/load from database.
    - **Session Management**: Clear Session Data button (with confirmation) to reset all user data, cart, orders, and preferences for fresh start.
    - **Auto-Refresh**: Profile statistics automatically update after checkout completion to reflect new purchases immediately.
